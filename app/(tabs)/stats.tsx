...
export default function StatsScreen() {
  ...
  const [selectedSlice, setSelectedSlice] = useState<CategoryBreakdown | null>(null);
  ...
  const loadAnalyticsData = async (selectedPeriod: Period) => {
    ...
    // Load category breakdown for expenses
    const expenseBreakdown = await getCategoryBreakdown('expense', selectedPeriod);
    const expenseWithColors = expenseBreakdown.map(item => {
      const category = getCategoriesByType('expense').find(c => c.name === item.category);
      return {
        ...item,
    ...
    setTopCategories(await getTopCategories(period));

    // Load time data based on period
  ...
  const hasData = !loading && summary && (summary.income > 0 || summary.expenses > 0);
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
        {loading ? (
          ...
          <>
            ...
            <ThemedView style={styles.section}>
              <ThemedText type="subtitle" style={styles.sectionTitle}>
                {period === 'thisYear' ? 'Monthly Trend' : 
                 period === 'thisMonth' ? 'Weekly Trend' : 'Daily Trend'}
              </ThemedText>
              {period === 'thisYear' ? (
